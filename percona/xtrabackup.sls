...
{%- set percona_xtrabackup_latest_version = salt['cmd.run_stdout']('curl -sL https://www.percona.com/downloads/XtraBackup/LATEST/ |grep -P "<option value=\\"XtraBackup\/Percona-XtraBackup-[^>]+>Percona-XtraBackup-[0-9\.]+</option>" |grep selected |sed -n "s~.*>Percona-XtraBackup-\([0-9\.]*\)</option>~\\1~p"', python_shell=True) %}

{# Get deb package URL of latest version #}
{%- set percona_xtrabackup_pkg_url = "https://www.percona.com/" ~ salt['cmd.run_stdout']('curl -sL -X POST -d "newBrowse=fetch-files&p=XtraBackup/Percona-XtraBackup-' ~ percona_xtrabackup_latest_version ~ '/binary/'~ grains['os_family'] | lower ~'/' ~ grains['oscodename'] | lower ~'" https://www.percona.com/downloads-ajax |grep -oP "/downloads/XtraBackup/Percona-XtraBackup-[^\s\\"]+' ~ mysql.tarball_os_arch | lower ~ '[^\s\\"]+percona-xtrabackup[-_][\d]+[^\s\\"]+\.deb"', python_shell=True) %}
